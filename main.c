...
int main(int argc, char** argv) {
    ...
    if (!file_exists("nofpga.opt")) {
        const char *FPGA_bitstream_path = "mc0:FPGA";
        if (file_exists("mc0:ACFPGALD") && file_exists(FPGA_bitstream_path)) {
            scr_setfontcolor(0x00F000);
            scr_printf("\t# ACFPGA driver found. uploading...\n");
            // dont take this as an example on how to pass arguments to an IRX... I use strlen bc it's a single arg
            // for multiple args, use sizeof, since arguments are expected as a null (\0) separated list, and the argc, the size of the buffer
            fpga.id = SifLoadStartModule("mc0:ACFPGALD", strlen(FPGA_bitstream_path), &FPGA_bitstream_path[0], &fpga.ret);
